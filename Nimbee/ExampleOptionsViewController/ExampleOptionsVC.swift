...
class ExampleOptionsVC: UIViewController {
    ...
    @objc func onBtnDoneTapped() {
        ...
        guard viewType == viewModel.perviousSelectedData.viewType else {
            ...
            switch viewType {
            case .defaultView:
                vc = mainStoryboard.instantiateViewController(withIdentifier: CalanderVC.className)
                (vc as? CalanderVC)?.viewModel.currentSelectedData = selectedData
            case .customView:
                vc = mainStoryboard.instantiateViewController(withIdentifier: CustomVC.className)
                (vc as? CustomVC)?.viewModel.currentSelectedData = selectedData
            case .longPressView:
                vc = mainStoryboard.instantiateViewController(withIdentifier: LongPressVC.className)
                (vc as? LongPressVC)?.viewModel.currentSelectedData = selectedData
            }
...
extension ExampleOptionsVC: UITableViewDelegate, UITableViewDataSource, ExpandableHeaderViewDelegate, OptionsCellDelegate {
